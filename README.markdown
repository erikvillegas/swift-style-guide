# raywenderlich.com Swift Style Guide (Carvana edition!)

Our overarching goals are readability, consistency and brevity, in that order.
...
* Set indent settings to 4 spaces to help with code readability. Be sure to set this preference in Xcode and in the Project settings.

* Method braces and other braces (`if`/`else`/`switch`/`while` etc.) should open on the next line as the statement and close on a new line. This makes code look less compressed and therefore easier to read.
...
Where an Xcode project is involved, the organization should be set to `Carvana` and the Bundle Identifier set to `com.carvana.MyAppName`.
